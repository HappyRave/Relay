...
| **Derived** | `recording`, `name`, `playback`, `loops`, `steps`, `moves`, `duration`, `desktop`, `frames`, `triggers`, `error`, `exportName` |
...
- **Actions** are arrow-function fields (`toggleRec`, `togglePlay`, `stop`, `seek`, `jump`, `edit`, `rename`, `insertWait`, `insertPixelCheck`, `pickPixel`, `setPlayback`, `setTriggers`, `duplicateMacro`, `deleteMacro`, `restoreMacro`, `importMacros`, `doExport`…), so they can be passed as event handlers without binding.
...
| `StepEditor` | Commits on `change` (blur or Enter), validates hex colors and numbers before sending an `EditOp`. |
| `HotkeyCapture` | Captures in the capture phase and stops propagation, so a combo being set never triggers anything else. Backspace clears, Esc cancels, blur cancels. |
